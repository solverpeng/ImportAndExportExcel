 ...
 * @author solverpeng
 */
public class ImportExcel_bk {

    private static Logger log = LoggerFactory.getLogger(ImportExcel_bk.class);
     ...
     */
    public ImportExcel_bk(String fileName, int headerNum) throws IOException {
     ...
     * @param headerNum 标题行号，数据行号=标题行号+1
     */
    public ImportExcel_bk(File file, int headerNum) throws IOException {
     ...
     */
    public ImportExcel_bk(String fileName, int headerNum, int sheetIndex) throws IOException {
     ...
     * @param sheetIndex 工作表编号
     */
    public ImportExcel_bk(File file, int headerNum, int sheetIndex) throws IOException {
     ...
     * @param sheetIndex 工作表编号
     */
    public ImportExcel_bk(MultipartFile multipartFile, int headerNum, int sheetIndex) throws IOException {
     ...
     * @param sheetIndex 工作表编号
     */
    public ImportExcel_bk(String fileName, InputStream is, int headerNum, int sheetIndex) throws IOException {
    ...
    public <E> List<E> getDataList(Class<E> clazz, int... groups) throws IllegalAccessException, InstantiationException {
        ...
        // 注释原因：只留下一种方式，即将注解标注实体类的属性，减少了方法的调用
        //handleAnnotationMethods(clazz, groups);
                    ...
                    try {
                        if (valType == String.class) {
                            ...
                            if (excelField.fieldType() != Class.class) {
                                cellValue = excelField.fieldType().getMethod("getValue", String.class).invoke(null, cellValue.toString());
                            } else {
                                cellValue = Class.forName(this.getClass().getName().replaceAll(this.getClass().getSimpleName(),
                                        "fieldtype." + valType.getSimpleName() + "Type")).getMethod("getValue", String.class).invoke(null, cellValue.toString());
                            }
                        }
                    } catch (Exception ex) {
    ...
    private void handleAnnotationMethods(Class<?> clazz, int[] groups) {
